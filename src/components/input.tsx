...
import { TextInput, TextInputProps, View, Platform, Text, Pressable } from "react-native";
...
function Input({ children, variant = "primary", label, isPassword = false, className }: InputProps) {
  return (
  ...
  return (
    <View className=" flex-row w-full p-2 rounded-lg border border-gray-appBorder">
      ...
      {isPassword && (
        <>
          {isShowPassword ? (
            <Eye onPress={toogleVisibilityPassword} size={16} color={colors.blue.app} />
          ) : (
            <EyeOff onPress={toogleVisibilityPassword} size={16} color={colors.blue.app} />
          )}
        </>
      )}
    </View>
